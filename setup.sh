...
configure_git() {
    ...
    if [[ "${SCRIPT_DIR}" =~ [Vv][Ii][Tt][Aa] || "${SCRIPT_DIR}" == */DATA*/* ]]; then
        log_info "https://gitlab.com/vita-linux/${project}"
        git -C "${SCRIPT_DIR}" config --local user.name "Vita-OS"
        git -C "${SCRIPT_DIR}" config --local user.email "vita-arch@proton.me"
        git -C "${SCRIPT_DIR}" remote set-url origin "git@gitlab.com:vita-linux/${project}.git"
        log_success "Git configured — remote set to git@gitlab.com:vita-linux/${project}"
    ...
    #     log_success "Git configured — remote set to git@github.com-kiro:kirodubes/${project}"
        ...
        log_error "Cannot determine identity — path contains neither VITA nor DATA: ${SCRIPT_DIR}"
        exit 1
    fi
}
